...
class Parser
{
    ...
    parseMoment()
    {
        ...
        {
            let time = new Date();
            return new Container(time.getTime(), time.getTimezoneOffset(), this.locale);
        }

        if(this.date instanceof Date)
        {
            return new Container(this.date.getTime(), this.date.getTimezoneOffset(), this.locale)
        }
    ...
    parseFromDate(year, month, date, hour, minute, second, milliSecond, offset)
    {
        ...
        return new Container(timeStamp+(
                Container.change(hour, "hours", "seconds")
                + Container.change(minute, "minutes", "seconds")
                + second
            )*1000
    ...
    parseWeekDateFormat(year, weekOfYear, day, hour, minute, second, milliSecond, offset){
        ...
        timeStamp += Container.change(hour, 'hours', 'seconds')*1000
            + Container.change(minute, 'minutes', 'seconds') * 1000
            + second*1000
            + milliSecond;

        return new Container(timeStamp, offset, this.locale);
    }
    ...
    parseOrdinalDateFormat(year, dayOfYear, hour, minute, second, milliSecond, offset){
        ...
        return new Container(timeStamp
            +(
                Container.change(dayOfYear-1, "days", "seconds")
                +Container.change(hour, "hours", "seconds")
                + Container.change(minute, "minutes", "seconds")
                + second
            )*1000
    ...
    parseToCustomFormatRegex(format) 
    {
        ...
        let regex = new RegExp(tempFormat);
    ...
    parseToCustomFormat(order, regex) 
    {
        ...
        if(this.userGivenInputTime.hasOwnProperty('x')){
            return new Container(this.userGivenInputTime['x'], 0, this.locale);
        }

        if(this.userGivenInputTime.hasOwnProperty('X')){
            return new Container(this.userGivenInputTime['X']*1000, 0, this.locale);
        }
